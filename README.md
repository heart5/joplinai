...
| 数据中心 | `aimod/center_api/` | 0.0.0.0:5003 | 腾讯云 | 增强缓存 / 历史DB / 笔记状态 / 用户管理 |
...
├── chunk_optimizer.py      # 自适应分块（待清理，chunk-and-embed 重构后已无引用）
...
- `aimod/` — AI 核心：文本分块与嵌入（`embedding_generator.py`、`chunk_optimizer.py`(待清理)、`text_splitter.py`）、ChromaDB 操作（`vector_db_manager.py`）、AI 缓存（`cache_manager.py`）、AI增强（`note_enhancer.py`）、运行追踪（`run_tracker.py`）
