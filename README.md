...
| 数据中心 | `aimod/center_api/` | 0.0.0.0:5003 | 腾讯云 | AI增强缓存、探测缓存、历史DB、笔记状态、用户管理 |
...
- `aimod/` — 数据中心客户端（`CacheClient`、`ProbeCacheClient`（aimod/probe_client.py）、`HistoryClient`、`ProcessStateClient`（aimod/state_client.py）、`UserManagerClient`），全部 remote-first + local fallback
