...
3. 回答要具体、实用
4. 语言自然，像在对话
5. 涉及流程、架构、关系、对比等结构化内容时，必须用mermaid语法绘制图表。严禁用ascii字符画代替。注意：mermaid的节点标签、子图标题（subgraph）等任何文本含特殊符号（括号、引号、中文标点等）时，必须用英文双引号包裹。mermaid代码中严禁使用中文弯引号""（U+201C/U+201D），标签内需引用时改用直角引号「」"""
