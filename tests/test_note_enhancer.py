...
class TestCacheFlow:
    ...
    def test_no_api_key_returns_none(self, de, monkeypatch):
        monkeypatch.setattr(de, "_get_cloud_api_key", lambda: "")
        result = de.cloud_process_note("测试文本", task="summary")
    ...
    def test_cache_hit_returns_cached(self, de, monkeypatch):
        ...
        result = de.cloud_process_note("测试文本", task="summary", use_cache=True)
        assert result == "缓存摘要"
    ...
    def test_cache_hit_with_validation_triggers_async_check(self, de, monkeypatch):
        ...
        with patch.object(de, "_validate_cache_entry_async") as mock_val:
            result = de.cloud_process_note("测试文本", task="tags", use_cache=True)
    ...
    def test_cache_miss_calls_api(self, de, monkeypatch):
        ...
        with patch.object(de, "_call_cloud_api", return_value="新结果") as mock_api:
    ...
    def test_bypass_cache_direct_api(self, de, monkeypatch):
        ...
        with patch.object(de, "_call_cloud_api", return_value="直调结果") as mock_api:
            result = de.cloud_process_note("测试", task="tags", use_cache=False)
            assert result == "直调结果"
            mock_api.assert_called_once()
...
class TestValidationAsync:
    ...
    def test_validation_updates_cache_on_change(self, de, monkeypatch):
        ...
        with patch.object(de, "_call_cloud_api", return_value="新验证结果"):
    ...
    def test_validation_no_change(self, de, monkeypatch):
        ...
        with patch.object(de, "_call_cloud_api", return_value="相同结果"):
            de._validate_cache_entry_async(
                original_text="原文",
                task="summary",
                cache_key="key456",
    ...
    def test_validation_api_failure(self, de, monkeypatch):
        ...
        with patch.object(de, "_call_cloud_api", return_value=None):
            de._validate_cache_entry_async(
                original_text="原文",
                task="summary",
                cache_key="key789",
    ...
    def test_validation_exception_handled(self, de, monkeypatch):
        ...
        with patch.object(de, "_call_cloud_api", side_effect=RuntimeError("boom")):
            de._validate_cache_entry_async(
                original_text="原文",
                task="summary",
                cache_key="key_err",
...
class TestCallAPI:
    def test_unsupported_task_returns_none(self, monkeypatch):
        ...
        monkeypatch.setattr(de, "_get_cloud_api_key", lambda: "fake-key")
        result = de._call_cloud_api("text", "invalid_task", "model", 1)
        assert result is None
